...
func Get_Project_by_Id(c *fiber.Ctx) error {
	...
	handlers.DB.Where("id =?", id).First(&project)
...
func Create_Project(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(400).
			JSON(fiber.Map{"error": "Invalid file logo", "details": err.Error()})
	}
	...
	if err := c.SaveFile(file, filePath); err != nil {
		return c.Status(500).
			JSON(fiber.Map{"error": "Failed to save file", "details": err.Error()})
	}
	...
	if err != nil {
		return c.Status(400).
			JSON(fiber.Map{"error": "Failed to parse multipart form"})
	}
	...
	if ok {
		for _, imageFile := range imageFiles {
			...
			if err := c.SaveFile(imageFile, imagePath); err != nil {
				return c.Status(500).
					JSON(fiber.Map{"error": "Failed to save image file", "details": err.Error()})
			}
	...
	if err := c.BodyParser(project); err != nil {
		return c.Status(400).
			JSON(fiber.Map{"error": "Invalid project data", "details": err.Error()})
	}

	// Create project in database
	if err := handlers.DB.Create(project).Error; err != nil {
		return c.Status(500).
			JSON(fiber.Map{"error": "Failed to create project", "details": err.Error()})
	}
...
func Delete_Project_func(c *fiber.Ctx) error {
	...
	var project models.Projects
	handlers.DB.Where("id =?", id).Delete(&project)
